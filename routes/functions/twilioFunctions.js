...
module.exports = {
    ...
    loadSigninView: function(req, res) {
        loadView("main/admin-signin", 200, genericSuccess, null, genericEmptyError, res);
    },
    ...
    receivedText: function(twiml, body, from, res) {
        retrieveFor('venue-management', 'users', 'phone', from, function(success, error, data) {
            if (!success) {
                ...
                processReceivedText(user, twiml, body, from, res);
            }
        });
    },

    createVenue: function() {
        //  Refactor code from index.js into this.
    }
}

function sendTextResponse(res, twiml, text) {
    twiml.message(text);
    res.writeHead(200, {'Content-Type': 'text/xml'});
    res.end(twiml.toString());
}

function processReceivedText(user, twiml, body, from, res) {
    if (body.startsWith('menu')) { // Show menu items
        ...
        if (body.includes('book')) { //  This is a request to learn how to book
            sendTextResponse(res, twiml, "To book text BOOK SECTION with the section number and the person's name. \n\nFor example, to book section 1 for Tracy Adams text the following: \n\nBOOK SECTION 1 for Tracy Adams");
            return
                ...
                Object.keys(venues).forEach(function(venueKey) {
                    ...
                    Object.keys(venue.sections).forEach(function(sectionKey) {
                        ...
                        if (obj.available === false) {
                            text += 'Section ' + obj.sectionTitle + ' booked by ' + obj.owner.firstname + obj.owner.lastname + '\n';
                        }
                    });
